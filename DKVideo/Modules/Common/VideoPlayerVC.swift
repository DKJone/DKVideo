...
//            playerView.playerConfig.maxCacheItem = isDownloaded ? 1 : 0
    ...
    override func viewDidLoad() {
        ...
        downloadBtn.setTitleColor(.primary(), for: [])
        ...
        downloadBtn.rx.tap.bind { [unowned self] in
            ...
            alert.addAction(title: "确定", style: .default, isEnabled: true) { [unowned self] _ in
                DownLoadManage.shared.addDownload(fileName: alert.textFields?.first?.text ?? defaultName, path: self.urlStr, autoStart: true)
